...
const ACADEMY_I18N_REDIRECTS: Record<string, string> = {
  ...
  // Landing "Inside Out" (casos-da-producao) → adiciona locale
  '/academy/casos-da-producao': '/pt/academy/casos-da-producao/',
  '/checkout/casos-da-producao': '/pt/checkout/casos-da-producao/',
}
...
export const config = {
  matcher: [
    '/((?!_next/static|_next/image|favicon.ico|sitemap.xml|robots.txt|site.webmanifest|downloads/|.*\\.(?:svg|png|jpg|jpeg|gif|webp|mp4|webm|woff|woff2|ttf|otf|ico|xml|txt|pdf|epub|webmanifest|json)$).*)',
  ],
}
