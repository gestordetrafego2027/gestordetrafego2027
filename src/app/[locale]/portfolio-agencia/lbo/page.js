...
export default function LboPage() {
    ...
    const horizontalImages = [
        ...
        "/images/agencia/lbo/lbo-consultoria-logo-marca-branding-house-mazzutti-agencia-design-07.webp"
    ];
    ...
    return (
        <div className="bg-surface text-on-surface antialiased">
                    ...
                    <div className="lg:w-2/3 flex flex-col gap-8">
                        {verticalImages.map((src, i) => (
                            <div key={i} className="image-anim relative overflow-hidden group cursor-pointer" onClick={() => openImg(i)} style={{height:'600px'}}>
                                <Image alt="LBO Consultoria — branding House Mazzutti" src={src} fill sizes="(max-width: 768px) 100vw, 66vw" quality={i===0?85:80} loading={i===0?undefined:"lazy"} priority={i===0?true:undefined} className="w-full h-full object-cover transition-all duration-700" />
                            </div>
                        ))}
                        {horizontalImages.map((src, i) => (
                            <div key={i} className="image-anim relative overflow-hidden group cursor-pointer" onClick={() => openImg(verticalImages.length + i)} style={{aspectRatio:'16/9'}}>
                                <Image alt="LBO Consultoria — identidade visual House Mazzutti" src={src} fill sizes="100vw" quality={80} loading={verticalImages.length===0 && i===0 ? undefined : "lazy"} priority={verticalImages.length===0 && i===0 ? true : undefined} className="object-cover w-full transition-all duration-700" />
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="lg:w-1/3">
                        <div className="info-col flex flex-col gap-[16px]">
                            <div className="space-y-1">
                                <p className="info-col-anim font-inter text-[10px] uppercase tracking-[0.2em] text-on-surface-variant">Logo, Marca e Branding</p>
                                <h1 className="info-col-anim font-newsreader text-6xl font-light -tracking-[0.02em] leading-tight text-on-surface hmzt-hero-title">LBO Consultoria</h1>
                            </div>
                            <div className="h-px w-12 bg-outline-variant/30"></div>
                            <p className="info-col-anim font-newsreader text-2xl italic leading-relaxed text-on-surface-variant">
                                &quot;Estratégia se reconhece pelo que ela sustenta.&quot;
                            </p>
                            <div className="space-y-[16px] pt-4">
                                ...
                                <div className="info-col-anim flex flex-col">
                                    ...
                                    <p className="font-inter text-sm font-medium">2019</p>
                                </div>
                            </div>
                            <div className="info-col-anim bg-white/50 p-8 mt-8 border border-outline-variant/10">
                                ...
                                <p className="font-inter text-xs text-on-surface-variant leading-relaxed">
                                    A LBO Consultoria precisava de uma marca que comunicasse clareza e autoridade. Desenvolvemos logotipo, símbolo e sistema de marca sobre uma base geométrica e sóbria — uma identidade que transmite método, confiança e visão de longo prazo em cada aplicação.
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <PortfolioVideo unit="agencia" slug="lbo" />

            <PortfolioCTA businessUnit="agencia" projectSlug="lbo" />
