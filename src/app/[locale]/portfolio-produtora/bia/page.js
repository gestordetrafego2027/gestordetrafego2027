...
export default function BiaPage() {
    ...
    const images = Array.from({length: 4}, (_, i) => `/images/produtora/acessorios/bia/${i+1}.webp`);
    ...
    return (
        <div className="bg-surface text-on-surface antialiased">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {images.map((src, i) => (
                            <div key={i} onClick={() => openImg(src)} className="cursor-pointer">
                              <Image alt={`Bia - Acessórios House Mazzutti - Foto ${i+1}`} style={{objectPosition: 'top'}} src={src} width={800} height={1200} sizes="(max-width: 768px) 100vw, 50vw" quality={85} loading="lazy" className="w-full aspect-[3/4] object-cover" />
                            </div>
                          ))}
                        </div>
                    </div>
                    <div className="lg:w-1/3">
                        <div className="info-col flex flex-col gap-[16px]">
                            <div className="space-y-1">
                                ...
                                <h1 className="info-col-anim font-newsreader text-6xl font-light -tracking-[0.02em] leading-tight text-on-surface hmzt-hero-title">Bia</h1>
                            </div>
            ...
            <PortfolioVideo unit="produtora" slug="bia" />

            <YouTubeShort urls="https://youtube.com/shorts/DLzTn9sSZ1s?feature=share" />

            <PortfolioCTA businessUnit="produtora" projectSlug="bia" />
