        ...
        <div className="bg-surface text-on-surface antialiased">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {images.map((src, i) => (
                            <div key={i} onClick={() => openImg(src)} className="cursor-pointer">
                              <Image alt={`TF - Institucional House Mazzutti - Foto ${i+1}`} style={{objectPosition: 'top'}} src={src} width={800} height={1200} sizes="(max-width: 768px) 100vw, 50vw" quality={85} loading="lazy" className="w-full aspect-[3/4] object-cover" />
                            </div>
                          ))}
                        </div>
                    </div>
                    <div className="lg:w-1/3">
                        <div className="info-col flex flex-col gap-[16px]">
                            <div className="space-y-1">
                                ...
                                <h1 className="info-col-anim font-newsreader text-6xl font-light -tracking-[0.02em] leading-tight text-on-surface hmzt-hero-title">TF</h1>
                            </div>
            ...
            {selectedImg && (
              <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center" onClick={closeImg}>
                <Image src={selectedImg} alt={`TF — House Mazzutti, imagem ampliada`} fill sizes="100vw" quality={80} loading="lazy" className="max-h-screen max-w-screen object-contain" />
              </div>
            )}
        </div>
    );
