...
export default function SuperbiaPage() {
    ...
    const images = Array.from({length: 7}, (_, i) => `/images/produtora/beleza/superbia/${i+1}.webp`);
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {images.map((src, i) => (
                            <div key={i} onClick={() => openImg(src)} className="cursor-pointer">
                              <Image alt={`Supérbia - Beleza House Mazzutti - Foto ${i+1}`} style={{objectPosition: 'top'}} src={src} width={800} height={1200} sizes="(max-width: 768px) 100vw, 50vw" quality={85} loading="lazy" className="w-full aspect-[3/4] object-cover" />
                            </div>
                          ))}
                        </div>
                    </div>
                    <div className="lg:w-1/3">
                        <div className="info-col flex flex-col gap-[16px]">
                            <div className="space-y-1">
                                ...
                                <h1 className="info-col-anim font-newsreader text-6xl font-light -tracking-[0.02em] leading-tight text-on-surface hmzt-hero-title">Supérbia</h1>
                            </div>
                            ...
                            <div className="space-y-[16px] pt-4">
                                ...
                                <div className="info-col-anim flex flex-col"><p className="font-inter text-[9px] uppercase tracking-[0.2em] text-outline-variant mb-1">Modelos</p><p className="font-inter text-sm font-medium">Barbara Nogueira &amp; Keila Fogaça</p></div>
                            </div>
                            <div className="info-col-anim bg-white/50 p-8 mt-8 border border-outline-variant/10">
                                ...
                                <p className="font-inter text-xs text-on-surface-variant leading-relaxed">Direção de beleza com foco em pele, luz e presença. Da supervisão de set ao corte final, padrão internacional.</p>
                            </div>
                        </div>
                    </div>
                </div>
