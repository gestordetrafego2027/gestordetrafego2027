...
export async function requestPasswordReset(formData: FormData): Promise<void> {
  ...
  if (!rl.allowed) {
    redirect(`/login/recuperar?error=${encodeURIComponent('Muitas tentativas. Aguarde um minuto e tente novamente.')}`)
  }

  const captcha = await verifyRecaptcha(recaptchaToken, 'recover')
  if (!captcha.ok) {
    redirect(`/login/recuperar?error=${encodeURIComponent('Falha na verificação de segurança. Tente novamente.')}`)
  }
  const origin =
