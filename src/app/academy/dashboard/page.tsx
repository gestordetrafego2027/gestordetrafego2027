...
export default async function StudentDashboardPage() {
  ...
  ;(
    ...
    if (
      p.last_watched_at &&
      (!cur.lastWatchedAt || new Date(p.last_watched_at) > new Date(cur.lastWatchedAt))
    ) {
  ...
  // Stats globais
  const totalSecondsWatched = Array.from(progByProduct.values()).reduce(
    (acc, p) => acc + p.totalSeconds,
    0,
  )
  const totalLessonsCompleted = Array.from(progByProduct.values()).reduce(
  ...
  const { data: lives } = await supabase
    .from('academy_lives')
    .select(
      'id, slug, title, scheduled_at, duration_minutes, cover_url, status, stream_url, product_id',
    )
  ...
  const bundles = enrolls.filter(
    (e) => e.product?.type === 'bundle' || e.product?.type === 'subscription',
  )
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <form action="/logout" method="post">
              <button className="rounded border border-neutral-300 px-3 py-1 text-xs hover:bg-neutral-100">
                Sair
              </button>
            </form>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <Kpi
            ...
            value={`${Math.floor(totalSecondsWatched / 3600)}h ${Math.floor((totalSecondsWatched % 3600) / 60)}m`}
          />
        ...
        {inProgress.length > 0 && (
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {inProgress.map((e) => {
                const p = progByProduct.get(e.product.id) ?? {
                  completedLessons: 0,
                  lastLessonId: null,
                  lastWatchedAt: null,
                  totalSeconds: 0,
                      ...
                      <div className="mt-3">
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className="text-neutral-500">
                            {p.completedLessons}/{total} aulas
                          </span>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs text-neutral-500">
                      ...
                      {l.status === 'live' && (
                        <span className="ml-2 rounded bg-rose-100 text-rose-700 px-1.5 py-0.5 text-[10px] font-medium">
                          AO VIVO
                        </span>
                      )}
                    </div>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              ...
              {completed.length === 0 ? (
                <p className="text-sm text-neutral-500 italic">
                  Termine seu primeiro curso para vê-lo aqui.
                </p>
              ) : (
                <ul className="space-y-2">
                  {completed.map((e) => (
                    <li
                      key={e.id}
                      className="rounded border border-neutral-200 bg-white px-4 py-2 flex items-center justify-between"
                    >
                      <span className="text-sm font-medium">{e.product.title}</span>
                      <Link
                        href={`/academy/curso/${e.product.slug}`}
            ...
            <div>
              ...
              {(certificates?.length ?? 0) === 0 ? (
                <p className="text-sm text-neutral-500 italic">
                  Conclua um curso para receber certificado.
                </p>
              ) : (
                <ul className="space-y-2">
                  {certificates!.map((c) => (
                    ...
                    >
                      <div>
                        <div className="text-sm font-medium">
                          {c.course_title_pt_snapshot ?? 'Certificado'}
                        </div>
                        <div className="text-xs text-neutral-500">
            ...
            <ul className="space-y-2">
              {liveEnrolls.map((e) => (
                <li key={e.id} className="rounded border border-neutral-200 bg-white px-4 py-2">
                  <Link
                    href={`/academy/live/${e.product.slug}`}
          ...
          <section className="space-y-3">
            ...
            <ul className="space-y-2">
              {bundles.map((e) => (
                <li
                  key={e.id}
                  className="rounded border border-neutral-200 bg-white px-4 py-2 flex justify-between"
                >
                  <span className="text-sm font-medium">{e.product.title}</span>
        ...
        {enrolls.length === 0 && (
          <section className="rounded-lg border-2 border-dashed border-neutral-200 bg-white p-8 text-center">
            <p className="text-neutral-600 mb-3">Você ainda não tem matrículas ativas.</p>
            <Link
              href="/academy"
              className="inline-block rounded bg-neutral-900 text-white text-sm px-5 py-2 hover:bg-neutral-700"
            >
              Explorar catálogo
            </Link>
          </section>
        ...
        <footer className="border-t border-neutral-200 pt-6 text-xs text-neutral-500 flex flex-wrap gap-4 justify-between">
          <Link href="/academy" className="hover:text-neutral-900">
            Catálogo
          </Link>
          <Link href="/academy/comunidade" className="hover:text-neutral-900">
            Comunidade
          </Link>
