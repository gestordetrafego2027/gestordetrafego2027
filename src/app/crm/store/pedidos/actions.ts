...
const UpdateSchema = z.object({
  ...
  status: z.enum(['pending','processing','paid','failed','refunded','partially_refunded','cancelled','chargeback']),
})
...
export async function updateOrderStatus(formData: FormData) {
  ...
  const { data: current } = await service
    .from('store_orders').select('status').eq('id', orderId).maybeSingle()
