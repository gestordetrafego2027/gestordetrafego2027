...
const TYPE_LABEL: Record<string, string> = {
  digital: 'Digital', service: 'Serviço', physical: 'Físico', bundle: 'Bundle',
}
const TYPE_COLOR: Record<string, string> = {
  digital: 'bg-blue-50 text-blue-700', service: 'bg-purple-50 text-purple-700',
  physical: 'bg-amber-50 text-amber-700', bundle: 'bg-emerald-50 text-emerald-700',
}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-400 mt-0.5">{active} ativos · {inactive} inativos</p>
        </div>
        <a
      ...
      {cats && cats.length > 0 && (
        <div className="flex gap-2 flex-wrap">
          {cats.map((c) => (
            <span key={c.id} className="px-3 py-1 text-xs border border-neutral-200 rounded-full text-neutral-600">
              {c.name}
            </span>
          ))}
        </div>
      ...
      <div className="grid gap-3">
        {!products?.length ? (
          ...
          products.map((product) => {
            ...
            const cats = (product.store_product_categories as any[])
            ...
            return (
              ...
              >
                {/* Status indicator */}
                <div className={`w-2 h-2 rounded-full flex-shrink-0 ${product.active ? 'bg-green-500' : 'bg-neutral-300'}`} />
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {product.featured && (
                      <span className="text-xs bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full border border-amber-100">destaque</span>
                    )}
                    <span className={`text-xs px-2 py-0.5 rounded-full ${TYPE_COLOR[product.product_type] ?? 'bg-neutral-100 text-neutral-600'}`}>
                  ...
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    ...
                    {cats.map((c: string) => (
                      <span key={c} className="text-xs text-neutral-400">· {c}</span>
                    ))}
                  </div>
                </div>
                ...
                <div className="text-right flex-shrink-0">
                  {mainPrice ? (
                    isQuoteOnly ? (
                      <span className="text-xs text-neutral-400 bg-neutral-100 px-2 py-1 rounded-lg">Orçamento</span>
                    ) : (
                      <p className="text-sm font-bold text-neutral-900">{fmt(mainPrice.unit_amount)}</p>
                    )
                  ) : (
                    <span className="text-xs text-neutral-300">sem preço</span>
                  )}
                  <p className="text-xs text-neutral-400 mt-0.5">{prices.length} preço{prices.length !== 1 ? 's' : ''}</p>
                </div>
      ...
      <p className="text-xs text-neutral-400 text-center">
        Produtos e preços são gerenciados diretamente no Stripe Dashboard e sincronizados via webhook ou script de sync.
      </p>
    </div>
  )
}
