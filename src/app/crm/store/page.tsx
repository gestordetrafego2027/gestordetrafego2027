...
export default async function StoreAdminPage() {
  ...
  const [{ data: kpis }, { data: recentOrders }, { count: pendingCount }, { count: catalogCount }] =
    await Promise.all([
      // Receita + pedidos do mês
      supabase.from('store_orders').select('total_cents, status').gte('created_at', startOfMonth),
      // Últimos 5 pedidos
      supabase
        .from('store_orders')
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <KpiCard
          ...
          href="/crm/store/pedidos?status=pending"
          sub="aguardando confirmação"
        />
        <KpiCard
          label="Produtos ativos"
          value={String(catalogCount ?? 0)}
          href="/crm/store/catalogo"
        />
        <KpiCard
          label="Ticket médio"
          value={orderCount > 0 ? fmt(Math.round(revenue / orderCount)) : '—'}
        />
      </div>
            ...
            recentOrders.map((order) => (
              <div key={order.id} className="flex items-center gap-4 px-6 py-3">
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_COLOR[order.status] ?? 'text-neutral-500 bg-neutral-100'}`}
                  >
