...
const activityIcon: Record<string, string> = {
  ...
  note: '📝', task: '✅', status_change: '🔄', stage_change: '➡️',
  quote_sent: '📄', system: '⚙️',
}
...
export default async function LeadDetailPage({
  ...
  const [
    ...
    supabase
      .from('leads')
      .select('*, pipeline_stages(name, slug)')
      .eq('id', id)
      .single(),
    supabase
      .from('activities')
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="rounded-lg border border-neutral-200 bg-white p-5">
            ...
            <form action={logActivityAction} className="mb-4 space-y-2 border-b border-neutral-100 pb-4">
              <input type="hidden" name="lead_id" value={lead.id} />
              <div className="flex gap-2">
                ...
                >
                  {ACTIVITY_TYPES.map((t) => (
                    <option key={t} value={t}>{activityIcon[t]} {t}</option>
                  ))}
                </select>
                <input
                  type="text"
        ...
        <aside className="space-y-4">
          ...
          {!!opps?.length && (
            <div className="rounded-lg border border-neutral-200 bg-white p-5">
              ...
              <ul className="space-y-2 text-sm">
                {opps.map((o) => (
                  <li key={o.id} className="flex justify-between border-l-2 border-emerald-300 pl-3">
                    <div>
