...
const ARTICLES = [
  {
    ...
    title: 'Book para modelos: o que realmente define quem é escolhido no mercado',
  },
  {
    slug: 'book-modelo-imagem-trabalha-por-voce',
    title: 'Book de modelo: quando sua imagem começa a trabalhar por você',
  },
  {
    slug: 'ensaio-pessoal-imagem-autoridade',
    title: 'Ensaio pessoal: o que realmente constrói uma imagem de autoridade',
  },
  {
    slug: 'ensaio-pessoal-imagem-lidera-percepcao',
    title: 'Ensaio pessoal: quando sua imagem deixa de acompanhar sua trajetória',
  },
  {
    slug: 'cobertura-externa-presenca-alto-valor',
    title: 'Cobertura externa em tempo real: presença de alto valor em São Paulo',
  },
  {
    slug: 'cobertura-externa-narrativa-visual',
    title: 'Cobertura externa em tempo real: quando São Paulo se transforma em narrativa visual',
  },
  {
    slug: 'branding-project-arquitetura-valor',
    title: 'Branding project como arquitetura de valor',
  },
  { slug: 'branding-project-motor-vendas', title: 'Branding project como motor de vendas' },
  {
    slug: 'quanto-investir-em-branding',
    title: 'Quanto investir em branding: o guia estratégico para empresas premium',
  },
  {
    slug: 'campanha-lancamento-arquitetura-invisivel',
    title: 'Campanha de lançamento: a arquitetura invisível das marcas que dominam atenção',
  },
  { slug: 'por-que-campanhas-falham', title: 'Por que a maioria das campanhas falha' },
  {
    slug: 'editorial-moda-narrativa-visual',
    title: 'Editorial de moda como narrativa visual: do produto ao desejo',
  },
  {
    slug: 'editorial-moda-performance-vendas',
    title: 'Editorial de moda orientado à performance: quando imagem passa a vender',
  },
  {
    slug: 'por-que-boas-ideias-nao-garantem-resultados',
    title: 'Por que boas ideias não garantem bons resultados',
  },
  {
    slug: 'producao-executiva-sistema-campanhas',
    title:
      'Produção executiva: o sistema invisível que transforma ideias em campanhas de alto impacto',
  },
  {
    slug: 'por-que-campanhas-caras-falham',
    title: 'Por que campanhas com alto investimento falham',
  },
]
...
export default function NewsletterPage() {
          ...
          from: (t: string) => {
            select: (c: string) => {
              order: (
                col: string,
                o: { ascending: boolean },
              ) => { limit: (n: number) => Promise<{ data: Send[] | null }> }
            }
          }
        }
      )
  ...
  async function handleSend() {
    ...
    try {
      ...
      setSendResult(
        res.ok
          ? { ok: true, total_sent: data.total_sent }
          : { ok: false, error: data.error ?? 'Erro desconhecido.' },
      )
      if (res.ok) {
        setSelectedSlug('')
        setCustomExcerpt('')
  ...
  async function toggleSubscriber(id: string, active: boolean) {
    ...
    await (supabase as unknown as SubClient)
      .from('newsletter_subscribers')
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-10">
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-6 space-y-5">
        <h2 className="text-[11px] uppercase tracking-[0.15em] text-gray-500 font-medium">
          Disparar nova carta
        </h2>
        ...
        <div>
          <label className="block text-[10px] uppercase tracking-[0.12em] text-gray-400 mb-2">
            Trecho de abertura do e-mail{' '}
            <span className="text-gray-300">(aparece no corpo antes do CTA)</span>
          </label>
          <textarea
            value={customExcerpt}
